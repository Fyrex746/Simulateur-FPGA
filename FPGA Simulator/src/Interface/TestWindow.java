...
public class TestWindow {
	...
	private void initialize() {
		...
		JTabbedPane AnsParamTab = new JTabbedPane(JTabbedPane.TOP);
		AnsParamTab.setVisible(false);
		frmTest.getContentPane().add(AnsParamTab, BorderLayout.CENTER);

		JPanel QCM = new JPanel();
		QCM.setVisible(false);
		AnsParamTab.addTab("New tab", null, QCM, null);
		QCM.setLayout(new GridLayout(0, 1, 0, 0));

		JCheckBox MultipleAnsw = new JCheckBox("R\u00E9ponses multiples");
		MultipleAnsw.setSelected(true);
		PossibleMultipleAnsw = MultipleAnsw.isSelected();
		QCM.add(MultipleAnsw);

		JPanel NbAnsPan = new JPanel();
		QCM.add(NbAnsPan);

		JLabel NbAns = new JLabel("Nombre de r\u00E9ponses");
		NbAnsPan.add(NbAns);

		NbQCMAns = new JTextField();
		NbAnsPan.add(NbQCMAns);
		NbQCMAns.setText(String.valueOf(NbPossibleAns));
		NbQCMAns.setColumns(10);

		JPanel Others = new JPanel();
		Others.setVisible(false);
		AnsParamTab.addTab("New tab", null, Others, null);

		JLabel lblTab_1 = new JLabel("Pas de r\u00E9glages possibles");
		Others.add(lblTab_1);
		...
		btnModifierLeSimulateur.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (result == JOptionPane.OK_OPTION) {
					 ...
					 * AnswNbBitsLUT.getText()); //si besoin affichage dans la
					 * console System.out.println("Nombre de LUTs: " +
					 * AnswNbLUT.getText());
		...
		btnParamtres.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				// JOptionPane.showMessageDialog(null, "Param\u00E8tres de la
				// zone");
				if (comboBox.getSelectedItem() == "QCM") {
					QCM.setVisible(true);
					int result = JOptionPane.showConfirmDialog(null, QCM, "", JOptionPane.OK_CANCEL_OPTION);
					if (result == JOptionPane.OK_OPTION) {
						PossibleMultipleAnsw = MultipleAnsw.isSelected();
						...
						if (PossibleMultipleAnsw) {
							...
							MultipleAnsw.setSelected(false);
						}
					}
					QCM.setVisible(false);
				} else {
					Others.setVisible(true);
					int result = JOptionPane.showConfirmDialog(null, Others, "", JOptionPane.CLOSED_OPTION);
					Others.setVisible(false);
				}
			}
		});
		...
		rdbtnmntmExamen.setSelected(true); // valeur par defaut du groupe de
											// boutons
